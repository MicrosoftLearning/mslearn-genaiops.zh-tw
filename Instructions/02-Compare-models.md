...
1. 回到 Azure 入口網站分頁，瀏覽至之前由部署指令碼建立的資源群組，然後選取您的 **Azure AI Foundry** 資源。
1. 在資源的 [概觀]**** 頁面中，選取 [按一下這裡以檢視端點]****，並複製 AI Foundry API 端點。
1. 將端點儲存在記事本中。 請使用該端點連線到用戶端應用程式中的專案。
...
1. 在程式代碼檔案中，以您先前複製之專案的端點取代 **your_project_endpoint** 預留位置。 觀察練習中使用的第一個和第二個模型，分別是 **gpt-4o** 和 **gpt-4o-mini**。
...
1. 執行最後一個指令碼之前，您必須從 Azure 入口網站複製 Azure AI Foundry 資源的資源識別碼。 移至 Azure AI Foundry 資源的概觀頁面，然後選取 [JSON 檢視]****。 複製資源識別碼，並取代程式碼檔案中的 `your_resource_id` 預留位置：
